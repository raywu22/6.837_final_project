...
void freeSystem() {
    ...
    delete waterSystem; waterSystem = nullptr;
}
...
void stepSystem()
{
    ...
    for (int i = 0; i < state.size() / 2; i++) {
        ...
        if (pos.x() <= TANK_START_X) {
            velocity = Vector3f(0.1f*abs(velocity.x()), velocity.y(), 0.0f);
            pos = Vector3f(TANK_START_X, pos.y(), 0.0f);
        }
        if (pos.x() >= TANK_END_X) {
            velocity = Vector3f(-0.1f*abs(velocity.x()), velocity.y(), 0.0f);
            pos = Vector3f(TANK_END_X, pos.y(), 0.0f);
        }
        if (pos.y() <= TANK_START_Y) {
            velocity = Vector3f(0.9f*velocity.x() + 1.0f*randNum/20.0f*abs(velocity.y()), 0.1 * abs(velocity.y()), 0.0f);
            pos = Vector3f(pos.x(), TANK_START_Y, 0.0f);
        }
