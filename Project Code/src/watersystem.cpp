...
const float PARTICLE_SPACING = 0.06f;
const float CELL_SPACING = 0.06f;
...
WaterSystem::WaterSystem()
{
    ...
    for (float x = TANK_START_X; x < -0.3f; x += PARTICLE_SPACING)
    for (float y = TANK_START_Y; y < TANK_END_Y+1.0f; y += PARTICLE_SPACING) {
       //populate the initial grid with current index
        int gridIndex = WaterSystem::posToGridIndex(x, y);
        Vector3f position = Vector3f(x, y + 0.5f, 0.0f);
...
//	fViscosity.print();
     //   cout << fPressure.abs() << " " << fViscosity.abs() << " " << fExternal.abs() << endl;
        ...
        if (fPressure.y() > 4.0f)
            fPressure = Vector3f(fPressure.x(), fPressure.y()/4.0f, 0.0f);
        Vector3f totalForce = 70.0f*fPressure + fViscosity + fGravity + fExternal;
